...
_G.PandaSpyBlacklist = {
    "UpdateCharacter", "UpdateMouse", "MoveDirection", "Animation", "Ping", "Stats"
}
...
if not _G.PandaHookNamecall then
    _G.PandaHookNamecall = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        -- MACRO RECORDER
        if State.Recording and not checkcaller() then
            if (method == "FireServer" or method == "InvokeServer") and typeof(self) == "Instance" then
                local isBl = false
                for _, b in ipairs(blacklist) do
                    if self.Name == b then isBl = true break end
                end
                if not isBl then
                    table.insert(_G.RecordedMacro, {
                        Remote = self,
                        Method = method,
                        Args = args,
                        Time = tick() - _G.MacroStartTime
                    })
                end
            end
        end
...
local function doFishing()
    _G.FishingSession = _G.FishingSession + 1
    local currentSession = _G.FishingSession
...
local function toggleAntiAFK()
    if State.AntiAFK then
        ...
        antiAfkConnection = LocalPlayer.Idled:Connect(function()
            ...
            warn("[Panda Anti-AFK]: Berhasil mencegah game dari disconnect (Idle 20 Menit)!")
        end)
        warn("[Panda Anti-AFK]: Aktif! Karakter Anda kebal dari kick AFK tanpa harus bergerak.")
    else
        if antiAfkConnection then
            antiAfkConnection:Disconnect()
            antiAfkConnection = nil
        end
        warn("[Panda Anti-AFK]: Dimatikan.")
    end
end
